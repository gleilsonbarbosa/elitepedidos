...
// Hook para buscar produtos mais pedidos da semana
export const useMostOrderedProducts = () => {
  const [mostOrdered, setMostOrdered] = useState<Array<{
    product_name: string;
    order_count: number;
  ...
  const fetchMostOrderedProducts = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      ...
      if (!supabaseUrl || !supabaseKey || 
          supabaseUrl.includes('placeholder') || 
          supabaseKey.includes('placeholder')) {
        console.warn('⚠️ Supabase não configurado - usando produtos populares de demonstração');
        
        // Mock data for demonstration
        const mockMostOrdered = [
          {
          ...
          {
            product_name: 'Açaí Premium 300g',
            order_count: 28,
            total_quantity: 42,
            image_url: 'https://images.pexels.com/photos/1092730/pexels-photo-1092730.jpeg?auto=compress&cs=tinysrgb&w=400',
            category: 'acai'
          ...
          {
            product_name: 'Milkshake de Açaí 500ml',
            order_count: 21,
            total_quantity: 25,
            image_url: 'https://images.pexels.com/photos/1092730/pexels-photo-1092730.jpeg?auto=compress&cs=tinysrgb&w=400',
            category: 'milkshake'
        ...
        setMostOrdered(mockMostOrdered);
        setLoading(false);
        return;
      }
      
      console.log('📊 Buscando produtos mais pedidos da semana...');
      
      // Calculate current week boundaries (Monday to Sunday)
      const now = new Date();
      const currentDay = now.getDay();
      const daysToSubtract = currentDay === 0 ? 6 : currentDay - 1; // Monday = 0 days to subtract
      ...
      console.log('📅 Período da semana:', {
        inicio: weekStart.toLocaleDateString('pt-BR'),
        fim: weekEnd.toLocaleDateString('pt-BR')
      });
      
      // Query to get most ordered products from this week
      const { data, error } = await supabase
        .from('orders')
        .select('items')
            ...
            if (productName) {
              if (!productCounts[productName]) {
                productCounts[productName] = {
                  count: 0,
                  quantity: 0,
                  image_url: item.product_image || 'https://images.pexels.com/photos/1092730/pexels-photo-1092730.jpeg?auto=compress&cs=tinysrgb&w=400',
                  category: 'acai' // Default category
                };
              }
              productCounts[productName].count += 1;
              productCounts[productName].quantity += item.quantity || 1;
            }
          });
        }
      });
      ...
      console.log(`✅ ${sortedProducts.length} produtos mais pedidos encontrados`);
      setMostOrdered(sortedProducts);
      
    } catch (err) {
      console.error('❌ Erro ao buscar produtos mais pedidos:', err);
      setError(err instanceof Error ? err.message : 'Erro ao carregar produtos populares');
  ...
  return {
    ...
    loading,
    error,
    refetch: fetchMostOrderedProducts
  };
};
/*
  # Corrigir registro de pagamento misto delivery no caixa

  ## Problema
  - Função link_delivery_order_to_cash_register também verifica 'money' mas o método é 'dinheiro'
  - Mantém consistência com a correção feita para vendas PDV

  ## Solução
  - Atualizar função para verificar 'dinheiro' em ambos os casos
*/
...
BEGIN
  ...
  ELSIF NEW.payment_method = 'mixed' AND NEW.mixed_payment_details IS NOT NULL THEN
    ...
    BEGIN
      ...
      LOOP
        IF payment_detail->>'method' IN ('money', 'dinheiro') THEN
